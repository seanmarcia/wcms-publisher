					...
					if (jQuery.Autocompleter !== undefined) {
						...
						$(data.fake_input).autocomplete({
						  source: function(query, response) {
						    return $.ajax($(".people_search").data('source'), {
						      data: {
						        query: query.term
						      },
						      dataType: "json",
						      success: function(data) {
						        return response($.map(data, function(person) {
						          return {
						...
						$(data.fake_input).data("ui-autocomplete")._renderItem = function(ul, item) {
					    var dep, img, link;
					    img = item.thumb ? $("<img>").attr("src", item.thumb) : '';
					    dep = item.department ? $("<div class='department'></div>").text(item.department) : '';
